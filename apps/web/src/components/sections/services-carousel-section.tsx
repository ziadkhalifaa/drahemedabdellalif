...
import { ChevronRight, ChevronLeft, Sparkles, ExternalLink } from 'lucide-react';
import { Link } from '@/i18n/routing';
import useSWR from 'swr';
import { api, getMediaUrl } from '@/lib/api';
import { useLocale } from 'next-intl';
import { cn } from '@/lib/utils';
import { TiltCard, FadeIn } from '@/components/motion/motion-utils';
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl relative z-10">
                ...
                <TiltCard tiltDegree={6} className="h-full">
                  ...
                  >
                    ...
                    <motion.div
                      ...
                      className="absolute top-5 left-5 z-20 w-9 h-9 rounded-xl bg-[var(--primary)] text-white text-xs font-black flex items-center justify-center shadow-lg"
                    >
                      0{index + 1}
                    </motion.div>
        ...
        <div className="flex items-center justify-center gap-6 mt-10">
          <motion.button
            onClick={scrollPrev}
            whileTap={{ scale: 0.9 }}
            className="md:hidden w-10 h-10 rounded-xl border border-[var(--border)] flex items-center justify-center text-[var(--muted)]"
          >
          ...
          <motion.button
            onClick={scrollNext}
            whileTap={{ scale: 0.9 }}
            className="md:hidden w-10 h-10 rounded-xl bg-[var(--primary)] flex items-center justify-center text-white"
          >
