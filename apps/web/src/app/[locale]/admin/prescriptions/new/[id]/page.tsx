...
export default function NewPrescriptionPage() {
  ...
  if (loading) return <div className="p-8 text-center text-sm font-bold">{isAr ? 'جاري التحميل...' : 'Loading...'}</div>;

  return (
    <div className="max-w-5xl mx-auto space-y-10 pb-20 relative" dir={isAr ? 'rtl' : 'ltr'}>
      ...
      <div className="relative flex flex-col md:flex-row md:items-center justify-between gap-8">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-2">
               ...
               <p className="text-xs font-bold text-[var(--muted)] uppercase tracking-widest opacity-70">
                 {isAr ? 'للمريض: ' : 'For Patient: '} <span className="text-[var(--foreground)]">{appointment?.patient?.name || appointment?.guestName}</span>
               </p>
            </div>
          </div>
        </div>
        ...
        >
          ...
          <span className="font-black uppercase tracking-widest text-xs">
            {isAr ? 'إصدار وحفظ الروشتة' : 'Issue & Save Prescription'}
          </span>
        </Button>
      </div>
      ...
      <div className="grid lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 space-y-8">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <Card className="p-8 bg-white/60 dark:bg-white/5 backdrop-blur-xl border-white/20 rounded-[2.5rem] shadow-xl shadow-black/5" dir="ltr">
              ...
              {medications.map((med, index) => (
                <div key={index} className="grid grid-cols-1 md:grid-cols-4 gap-4 p-6 bg-black/5 dark:bg-white/5 rounded-[2rem] border border-transparent hover:border-primary/20 transition-all group relative">
                  <div className="space-y-2">
                    <label className="text-[9px] font-black uppercase tracking-widest text-[var(--muted)] mx-1">
                  ...
                  <div className="space-y-2 flex gap-2 items-end">
                    ...
                    <Button 
                      ...
                      className="w-10 h-10 rounded-xl text-red-400 hover:bg-red-500/10 hover:text-red-500 transition-all shrink-0"
                    >
                      <Trash2 size={18} />
                    </Button>
                  </div>
